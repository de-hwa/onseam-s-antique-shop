...
$sql = "select * from tb_notice where no=$no";
...
$sqlf = "select * from tb_notice_file where notice_no=$no";
...
<table class="notice-view-table">
  ...
  <tr>
    ...
    <td>
      <?php if($rowf): ?>
        <a href="../file/download.php?board=notice&file_id=<?=$rowf['file_id']?>"><?=$rowf['original_name']?> [저장]</a>
      <?php endif; ?>
    </td>
  </tr>
</table>
...
?>
  <button type="button" class="btn" onclick="location.href='../board/notice_form.php?no=<?=$no?>'">글 수정</button>
  <form id="deleteForm" action="../board/notice_delete.php" method="POST" style="display:inline;">
