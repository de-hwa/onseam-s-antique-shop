...
$no = $_GET['no'] ?? '';
$is_new = ($no === '' || !ctype_digit($no));

$row = null;
if(!$is_new) {
        $stmt = $conn->prepare("select * from tb_notice where no = ?");
...
<?php if ($is_new):?>
<form class="form-box" action="../board/notice_insert.php" name="notice_form" id="notice_form" method="POST" onsubmit="return notice_check()" enctype="multipart/form-data">
  <div class="row">
  ...
  <div class="actions">
    ...
    <button type="button" class="btn" onclick="location.href='../board/notice_list.php'">목록으로</button>
  </div>
</form>

<?php else:?>
<form class="form-box" action="../board/notice_update.php" name="update_form" id="update_form" method="POST" onsubmit="return notice_check()" enctype="multipart/form-data">
